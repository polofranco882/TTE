...
router.get('/', authenticateToken, async (req: any, res: any) => {
    try {
        ...
        await client.query(`ALTER TABLE books ADD COLUMN IF NOT EXISTS sort_order INTEGER`);
...
router.get('/:id/contents', authenticateToken, async (req: any, res: any) => {
    try {
        ...
        await client.query(`ALTER TABLE book_contents ADD COLUMN IF NOT EXISTS is_active BOOLEAN DEFAULT TRUE`);
        await client.query(`ALTER TABLE book_contents ADD COLUMN IF NOT EXISTS parent_id INTEGER REFERENCES book_contents(id) ON DELETE SET NULL`);
